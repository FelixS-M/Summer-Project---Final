...
def listarraymaker():
	...
	Bound_Flat_List = np.loadtxt('Bound Flat.txt', dtype = {'names': ('KeplerID', 'planetnumber'), 'formats': (int, int)}, skiprows = 1)
	...
	for star in DATAarray: #loop over all the stars
		...
		lists = [Main_FP_List, Slightly_FP_List, CT_FP_List, Both_Flat_List, Bound_Flat_List] #A list of all the different lists
		names = ['Main_FP_', 'Slightly_FP_', 'CT_FP_', 'Both_Flat_', 'Bound_Flat_'] #Names for the files which will contain the list arrays
	...
	Bound_Flat_Array = np.load('Bound_Flat_dataarray.npy')
	Remain_Array = np.load('remainingdataarray.npy')
	print len(Remain_Array['KeplerID']), (len(DATAarray['KeplerID']) - len(Main_FP_Array['KeplerID']) - len(Slightly_FP_Array['KeplerID']) - len(CT_FP_Array['KeplerID']) - len(Both_Flat_Array['KeplerID']) - len(Bound_Flat_Array['KeplerID'])) #testing, confirming that there are no double entries
	return Main_FP_Array, Slightly_FP_Array, CT_FP_Array, Both_Flat_Array, Bound_Flat_Array, Remain_Array


def largebasicplotfull(Main_FP_Array, Slightly_FP_Array, CT_FP_Array, Both_Flat_Array, Bound_Flat_Array, Remain_Array):
	...
	f6 = ax.plot( Bound_Flat_Array['Signal-to-Noise'], Bound_Flat_Array['QualityofFit'], ',', label = ['Bound LC FLat'], color = 'yellow') #plot both sets of data
	...
	leg = ax.legend([f1,f2,f3,f4,f5,f6], ['Planetary Candidates', 'Main FP Selection', 'Possible FP Selection', 'Hard To/Cannot Tell', 'Both LC\'S Flat', 'Bound LC FLat'], fancybox=True, loc = 'upper left') #plot the legend, in the lower righthand corner
...
def multiplotfull(Main_FP_Array, Slightly_FP_Array, CT_FP_Array, Both_Flat_Array, Bound_Flat_Array, Remain_Array):
	fullarray = np.concatenate((Main_FP_Array, Slightly_FP_Array, CT_FP_Array, Both_Flat_Array, Bound_Flat_Array, Remain_Array), axis=0)
	...
	arrays = [Main_FP_Array, Slightly_FP_Array, CT_FP_Array, Both_Flat_Array, Bound_Flat_Array, Remain_Array]
	colours = ['red', 'pink', 'purple', 'orange', 'yellow', 'green']
	names = ['Main FP Selection', 'Possible FP Selection', 'Hard To_Cannot Tell', 'Both LC\'S Flat', 'Bound LC FLat', 'Planetary Candidates']
...
def largebasicplot_fitted(Main_FP_Array, Slightly_FP_Array, CT_FP_Array, Both_Flat_Array, Bound_Flat_Array, Remain_Array):
	...
	f2 = ax.plot( Main_FP_Array['Signal-to-Noise'], Main_FP_Array['QualityofFit'], ',', label = ['Main FP Selection'], color = 'red') #plot both sets of data
	ax.plot(xp,yp,'-', color='grey')
	leg = ax.legend([f1,f2,f3,f4,f5,f6], ['Planetary Candidates', 'Main FP Selection', 'Possible FP Selection', 'Hard To/Cannot Tell', 'Both LC\'S Flat', 'Bound LC FLat'], fancybox=True, loc = 'upper right') #plot the legend, in the lower righthand corner
	leg.get_frame().set_alpha(0.5) #make the legend slightly transparent
	plt.xlabel('log10(Signal-to-Noise Ratio)')
	plt.ylabel('log10(Quality of Fit)')
	plt.title('Signal-to-Noise Ratio vs Quality of Fit')
...
def main():
	...
	Main_FP_Array, Slightly_FP_Array, CT_FP_Array, Both_Flat_Array, Bound_Flat_Array, Remain_Array = listarraymaker()
	largebasicplotfull(Main_FP_Array, Slightly_FP_Array, CT_FP_Array, Both_Flat_Array, Bound_Flat_Array, Remain_Array)
	#largebasicplot_fitted(Main_FP_Array, Slightly_FP_Array, CT_FP_Array, Both_Flat_Array, Bound_Flat_Array, Remain_Array)
	multiplotfull(Main_FP_Array, Slightly_FP_Array, CT_FP_Array, Both_Flat_Array, Bound_Flat_Array, Remain_Array)
	return
