#Part of the Bound (Realistic stars) fit data pipeline

#This is the Second Stage in the data processing pipeline: It fits an analytical lightcurve to each transit
#This script stars by load the stellar and planetary parameters and using them to calculate an initial fit: Each fit requires Teff, Logg, Metalicity, Planetary radius and inclination 
#  - it is these values that are iterated over inorder to find the best fit.
#The realistic stars condition is forced by using the Torres relation to calculate Mstar and Rstar, thus forcing a relationship between Teff, Logg, Metalicity and the Limb Darkening 
# 	coefficients which results in realistic stars (or a failed fit when a realistic star cannot be found).
...
#This script also saves the fitted and original stellar parameters in new structured arrays along with a separate file which contains the analytical (fitted) lightcurve 
#NOTE: In order to ensure that mpfits is actually able to find a best fit, the initial guess for the inclination is always pi/2

#NOTE: This only differs from it's partner script in that it will NOT reprocess any stars which already have saved analytical transit lightcurves - It is used when dealing with large samples
#The code can be interrupted (command-c) and them resumed with very little performance loss - perfect for very large samples
...
def transitcalcloop(): #Just need to add the save functions
	...
	for line in plandataarray: #now loop through the planetary data array, in which line represents a single planet
		...
		if os.path.exists('kepler-{0}-planet-{1}-analyticaltransitlc.npy'.format(line['KeplerID'], line['planetnumber'])) == False:
			...
			Inclination = math.acos(line['Impactpar'] / line['a/R_star']) #calculate the inclination from the normalised impact-parameter
			#print Inclination
			...
			chi, chireduced, DOF, chifull, chireducedfull, DOFfull = chicalc(Keplertransit, analyticaltransitlc, sigma, T_dur_SI) #calculate the chi-squared values
...
def zandtfit(Inclination, Planetaryradius_SI, Keplerlc, Teff, Logg, Metalicity, sigma, Orbitalperiod_SI, KeplerID, Planetnumberer):
	...
	p0 = [1.57, Planetaryradius_Earth, Teff, Logg, np.log10(Metalicity)] #inital guess (from kepler mast and headers)
	...
	parinfo = [{'value':p0[0], 'fixed':0, 'limited':[1,1], 'limits':[0,1.570796], 'step':0}, {'value':p0[1], 'fixed':0, 'limited':[0,0], 'limits':[0.01,100], 'step':0},\
	...
	m = mpfit(minimisefunction, p0, functkw = fa, quiet = 0, maxiter = 30, parinfo = parinfo, ftol=0.0001) #run the mpfit for the best fit
	#print m #testing
	...
	if covar == None: #if no covariance array is produced, create an array of zeros so that there are no problems later
		covar = np.zeros((5,5))
	#print bestfitarray
	Inclination_fit = bestfitarray[0] #Extract the fit inclination
...
def minimisefunction(x, Keplerlc=None, fjac = None, sigma = None, Orbitalperiod_SI = None, KeplerID = None, Planetnumberer = None): #DONE
	Inclination = x[0] #extract the impact parameter
...
def chicalc(Keplerlc, analyticallc, sigma, T_dur_SI): #calculates the chi^2 and reduced chi^2 values whilst only using those data points that fall within a transit #Still should be ok
	...
	return chi2, chi2reduced, count, chi2full, chi2reducedfull, countfull #return both chi^2 values 
